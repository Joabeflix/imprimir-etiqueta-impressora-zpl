...
def imprimir(zpl_command, printer_name):
...
def conteudo_imprimir():
    ...
    # "loop_impressao".
    texto_imprimir = texto_imprimir_input.get()
...
# Verificador se vai usar imprimir o código de barras
    usar_ou_nao_codigo_barras = check_var_codigo_barras.get()
...
def loop_impressao(qtd):

    # Verificando o conteúdo para imprimir
    conteudo_imprimir_valor = conteudo_imprimir()
    ...
    if conteudo_imprimir_valor is None:
        ...
        for x in range(qtd_numero):
            ...
            imprimir(conteudo_imprimir_valor, nome_impressora_())
...
lista_impressoras = impressoras_no_sistema()
...
#________________________________________________________________________________________________#
# Labels do programa
tk.Label(root, text="Selecionar impressora:").place(x=63, y=2)
tk.Label(root, text="Código para imprimir").place(x=22, y=65)
tk.Label(root, text="Quantidade").place(x=168, y=65)
...
botao = ttk.Button(root, text="Imprimir", width=12, command=lambda: loop_impressao(qtd_pag()), bootstyle="success", )
botao.place(x=77, y=210)

#________________________________________________________________________________________________#
# Entrada di texto para imprimir
texto_imprimir_input = ttk.Entry(root, width=20, bootstyle="success")
texto_imprimir_input.place(x=22, y=85)

#________________________________________________________________________________________________#
# Entrada da quantidade de páginas para imprimir
quantidade_pag = ttk.Entry(root, width=8, bootstyle="info")
quantidade_pag.place(x=174, y=85)

#________________________________________________________________________________________________#
# Criar uma BooleanVar para armazenar o estado do Checkbutton
check_var_codigo_barras = tk.BooleanVar()
# Criar o Checkbutton usando a BooleanVar
check_codigo_barras = ttk.Checkbutton(root, text="Código de barras", variable=check_var_codigo_barras)
check_codigo_barras.place(x=10, y=130)
